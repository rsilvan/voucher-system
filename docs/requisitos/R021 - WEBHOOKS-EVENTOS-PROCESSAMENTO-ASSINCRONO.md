...
Este requisito tem como objetivo permitir integração reativa e confiável com sistemas externos e processamento assíncrono tolerante a falhas. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.

## 2. Escopo

### Incluído
- webhooks de projeto e distribuição.
- catálogo versionado de eventos.
- assinatura HMAC, timestamp e replay protection.
- filtros por evento.
...
### Fora de escopo
- fila gerenciada externa como produto.
- streaming analytics complexo.
- ordenação global absoluta de todos eventos.
- garantia exactly-once externa.
...
3. Ator cria ou consulta recurso de eventos internos, outbox, webhooks e workers com payload válido.
...
Fluxos alternativos:
1. Ator executa commit de outbox na transação usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r021:read`.
- Escrita: `r021:write`.
- Execução de ação crítica: `r021:execute`.
- Administração: `r021:admin`.
- Auditoria/exportação: `r021:audit` ou `r021:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de eventos internos, outbox, webhooks e workers deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como commit de outbox na transação, assinatura HMAC, entrega com retry só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de eventos internos, outbox, webhooks e workers devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de eventos internos, outbox, webhooks e workers.  
  Prioridade: Must.  
  Dependências: R002, R003.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como commit de outbox na transação, assinatura HMAC, entrega com retry, dead-letter, replay manual.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de eventos internos, outbox, webhooks e workers devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/webhooks-eventos-processamento-assincrono` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `WebhookEndpoint`: destino. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `WebhookSubscription`: assinatura. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `OutboxMessage`: evento interno. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `WebhookDelivery`: entrega. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `InboxMessage`: deduplicação de consumer. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `DeadLetter`: falha final. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/webhooks`  
  Objetivo: listar recursos de eventos internos, outbox, webhooks e workers com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r021:read`.  
...
- `POST /api/v1/webhooks`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r021:write`.  
...
- `GET /api/v1/webhooks/{id}`  
  ...
  Autenticação exigida: scope `r021:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/webhooks/{id}`  
  ...
  Autenticação exigida: scope `r021:write`.  
...
- **`webhook_endpoint.created`**  
  Quando ocorre: após commit de outbox na transação ou mudança relevante no domínio de eventos internos, outbox, webhooks e workers.  
...
Rotas:
- `/developers/webhooks`: listagem principal.
- `/developers/webhooks/new`: criação.
- `/developers/webhooks/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/developers/webhooks/:id/edit`: edição controlada por estado.
- `/developers/webhooks/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de eventos internos, outbox, webhooks e workers não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar commit de outbox na transação; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r021_requests_total`
- `voucher_system_r021_failures_total`
- `voucher_system_r021_operation_duration_ms`
- `voucher_system_r021_business_events_total`
...
- Custom event `R021.WebhooksEventosEProcessamentoAssincrono.OperationCompleted`.
- Custom event `R021.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para eventos internos, outbox, webhooks e workers.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para eventos internos, outbox, webhooks e workers estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/WebhooksEventosProcessamentoAssincrono/...`
- `src/VoucherSystem.Application/WebhooksEventosProcessamentoAssincrono/Commands/...`
- `src/VoucherSystem.Application/WebhooksEventosProcessamentoAssincrono/Queries/...`
- `src/VoucherSystem.Api/Endpoints/WebhooksEventosProcessamentoAssincrono/...`
...
- `src/VoucherSystem.Workers/WebhooksEventosProcessamentoAssincrono/...`
- `frontend/src/features/webhooks-eventos-processamento-assincrono/...`
- `frontend/src/routes/...`
- `docs/requisitos/R021 - WEBHOOKS-EVENTOS-PROCESSAMENTO-ASSINCRONO.md`
- `tests/VoucherSystem.UnitTests/WebhooksEventosProcessamentoAssincrono/...`
- `tests/VoucherSystem.IntegrationTests/WebhooksEventosProcessamentoAssincrono/...`
- `tests/VoucherSystem.ApiTests/WebhooksEventosProcessamentoAssincrono/...`
- `frontend/tests/webhooks-eventos-processamento-assincrono/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para eventos internos, outbox, webhooks e workers.  
