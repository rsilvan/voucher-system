...
Este requisito tem como objetivo transformar clientes em promotores, medir conversões e recompensar referrer e referee de forma segura. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
- join-once.
- conversão por resgate ou evento.
- vínculo referrer/referee.
- bloqueio de autorreferência.
- tiers por conversão ou marco acumulado.
- rewards digitais e materiais.
- antifraude básico.

### Fora de escopo
- rede social de indicações.
- tracking publicitário externo avançado.
- detecção antifraude por IA.
...
3. Ator cria ou consulta recurso de indicação, conversão e recompensas por referral com payload válido.
...
Fluxos alternativos:
1. Ator executa criação de código usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r018:read`.
- Escrita: `r018:write`.
- Execução de ação crítica: `r018:execute`.
- Administração: `r018:admin`.
- Auditoria/exportação: `r018:audit` ou `r018:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de indicação, conversão e recompensas por referral deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como criação de código, join do referee, detecção idempotente de conversão só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de indicação, conversão e recompensas por referral devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de indicação, conversão e recompensas por referral.  
  Prioridade: Must.  
  Dependências: R004, R007, R017.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como criação de código, join do referee, detecção idempotente de conversão, bloqueio de autoindicação, concessão de recompensa.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de indicação, conversão e recompensas por referral devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/referral-conversoes-indicacao` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `ReferralProgram`: programa. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ReferralCode`: código. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ReferralParticipant`: participante. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ReferralRelationship`: vínculo. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ReferralConversion`: conversão. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ReferralReward`: recompensa. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/referrals`  
  Objetivo: listar recursos de indicação, conversão e recompensas por referral com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r018:read`.  
...
- `POST /api/v1/referrals`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r018:write`.  
...
- `GET /api/v1/referrals/{id}`  
  ...
  Autenticação exigida: scope `r018:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/referrals/{id}`  
  ...
  Autenticação exigida: scope `r018:write`.  
...
- **`referral_code.created`**  
  Quando ocorre: após criação de código ou mudança relevante no domínio de indicação, conversão e recompensas por referral.  
...
- **`referral_conversion.detected`**  
  Quando ocorre: após criação de código ou mudança relevante no domínio de indicação, conversão e recompensas por referral.  
  Payload mínimo: `eventId`, `eventType`, `occurredAt`, `accountId`, `projectId`, `entityId`, `entityType`, `actorId`, `correlationId`, `version`, `metadata`.  
  Deve gerar webhook: sim, quando o projeto tiver assinatura ativa para esse evento.  
  Deve entrar em outbox: sim, obrigatoriamente na mesma transação da mudança.

- **`referral_reward.granted`**  
  Quando ocorre: após criação de código ou mudança relevante no domínio de indicação, conversão e recompensas por referral.  
  Payload mínimo: `eventId`, `eventType`, `occurredAt`, `accountId`, `projectId`, `entityId`, `entityType`, `actorId`, `correlationId`, `version`, `metadata`.  
  Deve gerar webhook: sim, quando o projeto tiver assinatura ativa para esse evento.  
  Deve entrar em outbox: sim, obrigatoriamente na mesma transação da mudança.
...
- Ação: criação de código.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de indicação, conversão e recompensas por referral.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: join do referee.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de indicação, conversão e recompensas por referral.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: detecção idempotente de conversão.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de indicação, conversão e recompensas por referral.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: bloqueio de autoindicação.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de indicação, conversão e recompensas por referral.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: concessão de recompensa.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de indicação, conversão e recompensas por referral.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/referrals`: listagem principal.
- `/referrals/new`: criação.
- `/referrals/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/referrals/:id/edit`: edição controlada por estado.
- `/referrals/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de indicação, conversão e recompensas por referral não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar criação de código; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r018_requests_total`
- `voucher_system_r018_failures_total`
- `voucher_system_r018_operation_duration_ms`
- `voucher_system_r018_business_events_total`
...
- Custom event `R018.ReferralEConversoesDeIndicacao.OperationCompleted`.
- Custom event `R018.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para indicação, conversão e recompensas por referral.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para indicação, conversão e recompensas por referral estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/ReferralConversoesIndicacao/...`
- `src/VoucherSystem.Application/ReferralConversoesIndicacao/Commands/...`
- `src/VoucherSystem.Application/ReferralConversoesIndicacao/Queries/...`
- `src/VoucherSystem.Api/Endpoints/ReferralConversoesIndicacao/...`
...
- `src/VoucherSystem.Workers/ReferralConversoesIndicacao/...`
- `frontend/src/features/referral-conversoes-indicacao/...`
- `frontend/src/routes/...`
- `docs/requisitos/R018 - REFERRAL-CONVERSOES-INDICACAO.md`
- `tests/VoucherSystem.UnitTests/ReferralConversoesIndicacao/...`
- `tests/VoucherSystem.IntegrationTests/ReferralConversoesIndicacao/...`
- `tests/VoucherSystem.ApiTests/ReferralConversoesIndicacao/...`
- `frontend/tests/referral-conversoes-indicacao/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para indicação, conversão e recompensas por referral.  
