...
Este requisito tem como objetivo detectar problemas antes que afetem campanhas e reduzir tempo de diagnóstico e recuperação. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
### Fora de escopo
- observability SaaS externo obrigatório.
- chaos engineering completo.
- DR multi-region automático.
- NOC 24x7.
...
3. Ator cria ou consulta recurso de telemetria, SLO, confiabilidade e operação com payload válido.
...
Fluxos alternativos:
1. Ator executa propagação de correlationId usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r029:read`.
- Escrita: `r029:write`.
- Execução de ação crítica: `r029:execute`.
- Administração: `r029:admin`.
- Auditoria/exportação: `r029:audit` ou `r029:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de telemetria, SLO, confiabilidade e operação deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como propagação de correlationId, emissão de métrica customizada, alerta por SLO só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de telemetria, SLO, confiabilidade e operação devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de telemetria, SLO, confiabilidade e operação.  
  Prioridade: Must.  
  Dependências: R021, R027.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como propagação de correlationId, emissão de métrica customizada, alerta por SLO, consulta de DLQ, execução de runbook.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de telemetria, SLO, confiabilidade e operação devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/observabilidade-confiabilidade-operacao` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `TelemetryEvent`: evento de telemetria. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `HealthCheckResult`: health. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `SloDefinition`: SLO. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `AlertRule`: alerta. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `Runbook`: runbook. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `WorkerStatus`: status. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/operations`  
  Objetivo: listar recursos de telemetria, SLO, confiabilidade e operação com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r029:read`.  
...
- `POST /api/v1/operations`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r029:write`.  
...
- `GET /api/v1/operations/{id}`  
  ...
  Autenticação exigida: scope `r029:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/operations/{id}`  
  ...
  Autenticação exigida: scope `r029:write`.  
...
- **`health_check.failed`**  
  Quando ocorre: após propagação de correlationId ou mudança relevante no domínio de telemetria, SLO, confiabilidade e operação.  
...
- Ação: alerta por SLO.  
  ...
  correlationId: obrigatório.  
  Severidade: High.
- Ação: consulta de DLQ.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de telemetria, SLO, confiabilidade e operação.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: execução de runbook.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de telemetria, SLO, confiabilidade e operação.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/operations`: listagem principal.
- `/operations/new`: criação.
- `/operations/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/operations/:id/edit`: edição controlada por estado.
- `/operations/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de telemetria, SLO, confiabilidade e operação não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar propagação de correlationId; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r029_requests_total`
- `voucher_system_r029_failures_total`
- `voucher_system_r029_operation_duration_ms`
- `voucher_system_r029_business_events_total`
...
- Custom event `R029.ObservabilidadeConfiabilidadeEOperacao.OperationCompleted`.
- Custom event `R029.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para telemetria, SLO, confiabilidade e operação.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para telemetria, SLO, confiabilidade e operação estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/ObservabilidadeConfiabilidadeOperacao/...`
- `src/VoucherSystem.Application/ObservabilidadeConfiabilidadeOperacao/Commands/...`
- `src/VoucherSystem.Application/ObservabilidadeConfiabilidadeOperacao/Queries/...`
- `src/VoucherSystem.Api/Endpoints/ObservabilidadeConfiabilidadeOperacao/...`
...
- `src/VoucherSystem.Workers/ObservabilidadeConfiabilidadeOperacao/...`
- `frontend/src/features/observabilidade-confiabilidade-operacao/...`
- `frontend/src/routes/...`
- `docs/requisitos/R029 - OBSERVABILIDADE-CONFIABILIDADE-OPERACAO.md`
- `tests/VoucherSystem.UnitTests/ObservabilidadeConfiabilidadeOperacao/...`
- `tests/VoucherSystem.IntegrationTests/ObservabilidadeConfiabilidadeOperacao/...`
- `tests/VoucherSystem.ApiTests/ObservabilidadeConfiabilidadeOperacao/...`
- `frontend/tests/observabilidade-confiabilidade-operacao/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para telemetria, SLO, confiabilidade e operação.  
