...
Este requisito tem como objetivo aplicar incentivos de forma consistente, impedir duplicidade e permitir reversões seguras em cancelamentos/devoluções. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
- estados pending/succeeded/failed/rolled_back.
- idempotency key.
- rollback integral ou compatível.
...
3. Ator cria ou consulta recurso de redemption, rollback e consistência transacional com payload válido.
...
Fluxos alternativos:
1. Ator executa commit de resgate usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r013:read`.
- Escrita: `r013:write`.
- Execução de ação crítica: `r013:execute`.
- Administração: `r013:admin`.
- Auditoria/exportação: `r013:audit` ou `r013:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de redemption, rollback e consistência transacional deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como commit de resgate, retry idempotente, rollback só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de redemption, rollback e consistência transacional devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de redemption, rollback e consistência transacional.  
  Prioridade: Must.  
  Dependências: R012.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como commit de resgate, retry idempotente, rollback, ajuste de limites/saldos, lock concorrente.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de redemption, rollback e consistência transacional devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/resgates-reversoes-idempotencia` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Redemption`: resgate. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RedemptionItem`: item aplicado. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RedemptionAttempt`: tentativa. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `Rollback`: reversão. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `IdempotencyRecord`: controle idempotente. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `OperationalCounter`: contador. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/redemptions`  
  Objetivo: listar recursos de redemption, rollback e consistência transacional com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r013:read`.  
...
- `POST /api/v1/redemptions`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r013:write`.  
...
- `GET /api/v1/redemptions/{id}`  
  ...
  Autenticação exigida: scope `r013:read` e acesso ao `account_id/project_id`.  
...
- **`redemption.started`**  
  Quando ocorre: após commit de resgate ou mudança relevante no domínio de redemption, rollback e consistência transacional.  
...
- Ação: rollback.  
  ...
  correlationId: obrigatório.  
  Severidade: High.
- Ação: ajuste de limites/saldos.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de redemption, rollback e consistência transacional.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: lock concorrente.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de redemption, rollback e consistência transacional.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/redemptions`: listagem principal.
- `/redemptions/new`: criação.
- `/redemptions/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/redemptions/:id/edit`: edição controlada por estado.
- `/redemptions/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de redemption, rollback e consistência transacional não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar commit de resgate; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r013_requests_total`
- `voucher_system_r013_failures_total`
- `voucher_system_r013_operation_duration_ms`
- `voucher_system_r013_business_events_total`
...
- Custom event `R013.ResgatesReversoesEIdempotencia.OperationCompleted`.
- Custom event `R013.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para redemption, rollback e consistência transacional.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para redemption, rollback e consistência transacional estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/ResgatesReversoesIdempotencia/...`
- `src/VoucherSystem.Application/ResgatesReversoesIdempotencia/Commands/...`
- `src/VoucherSystem.Application/ResgatesReversoesIdempotencia/Queries/...`
- `src/VoucherSystem.Api/Endpoints/ResgatesReversoesIdempotencia/...`
...
- `src/VoucherSystem.Workers/ResgatesReversoesIdempotencia/...`
- `frontend/src/features/resgates-reversoes-idempotencia/...`
- `frontend/src/routes/...`
- `docs/requisitos/R013 - RESGATES-REVERSOES-IDEMPOTENCIA.md`
- `tests/VoucherSystem.UnitTests/ResgatesReversoesIdempotencia/...`
- `tests/VoucherSystem.IntegrationTests/ResgatesReversoesIdempotencia/...`
- `tests/VoucherSystem.ApiTests/ResgatesReversoesIdempotencia/...`
- `frontend/tests/resgates-reversoes-idempotencia/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para redemption, rollback e consistência transacional.  
