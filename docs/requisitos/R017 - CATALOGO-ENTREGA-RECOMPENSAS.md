...
Este requisito tem como objetivo reutilizar recompensas em loyalty e referral, controlando custo, disponibilidade, estoque e entrega. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
- auto-redeem protegido.

### Fora de escopo
- WMS/logística completa.
- marketplace externo.
...
3. Ator cria ou consulta recurso de recompensas, catálogo e fulfillment com payload válido.
...
Fluxos alternativos:
1. Ator executa criação de recompensa usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r017:read`.
- Escrita: `r017:write`.
- Execução de ação crítica: `r017:execute`.
- Administração: `r017:admin`.
- Auditoria/exportação: `r017:audit` ou `r017:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de recompensas, catálogo e fulfillment deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como criação de recompensa, assignment a programa/tier, resgate com débito atômico só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de recompensas, catálogo e fulfillment devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de recompensas, catálogo e fulfillment.  
  Prioridade: Must.  
  Dependências: R006, R015, R016.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como criação de recompensa, assignment a programa/tier, resgate com débito atômico, emissão digital, fulfillment.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de recompensas, catálogo e fulfillment devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/catalogo-entrega-recompensas` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Reward`: recompensa. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RewardCatalog`: catálogo. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RewardAssignment`: vínculo. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RewardInventory`: estoque. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RewardRedemption`: resgate. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `FulfillmentOrder`: entrega. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/rewards`  
  Objetivo: listar recursos de recompensas, catálogo e fulfillment com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r017:read`.  
...
- `POST /api/v1/rewards`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r017:write`.  
...
- `GET /api/v1/rewards/{id}`  
  ...
  Autenticação exigida: scope `r017:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/rewards/{id}`  
  ...
  Autenticação exigida: scope `r017:write`.  
...
- **`reward.created`**  
  Quando ocorre: após criação de recompensa ou mudança relevante no domínio de recompensas, catálogo e fulfillment.  
...
- Ação: criação de recompensa.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de recompensas, catálogo e fulfillment.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: assignment a programa/tier.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de recompensas, catálogo e fulfillment.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: resgate com débito atômico.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de recompensas, catálogo e fulfillment.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: emissão digital.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de recompensas, catálogo e fulfillment.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/rewards`: listagem principal.
- `/rewards/new`: criação.
- `/rewards/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/rewards/:id/edit`: edição controlada por estado.
- `/rewards/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de recompensas, catálogo e fulfillment não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar criação de recompensa; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r017_requests_total`
- `voucher_system_r017_failures_total`
- `voucher_system_r017_operation_duration_ms`
- `voucher_system_r017_business_events_total`
...
- Custom event `R017.CatalogoEEntregaDeRecompensas.OperationCompleted`.
- Custom event `R017.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para recompensas, catálogo e fulfillment.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para recompensas, catálogo e fulfillment estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/CatalogoEntregaRecompensas/...`
- `src/VoucherSystem.Application/CatalogoEntregaRecompensas/Commands/...`
- `src/VoucherSystem.Application/CatalogoEntregaRecompensas/Queries/...`
- `src/VoucherSystem.Api/Endpoints/CatalogoEntregaRecompensas/...`
...
- `src/VoucherSystem.Workers/CatalogoEntregaRecompensas/...`
- `frontend/src/features/catalogo-entrega-recompensas/...`
- `frontend/src/routes/...`
- `docs/requisitos/R017 - CATALOGO-ENTREGA-RECOMPENSAS.md`
- `tests/VoucherSystem.UnitTests/CatalogoEntregaRecompensas/...`
- `tests/VoucherSystem.IntegrationTests/CatalogoEntregaRecompensas/...`
- `tests/VoucherSystem.ApiTests/CatalogoEntregaRecompensas/...`
- `frontend/tests/catalogo-entrega-recompensas/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para recompensas, catálogo e fulfillment.  
