...
Este requisito tem como objetivo permitir configurar e explicar quem, quando, onde e em qual contexto pode receber ou utilizar um incentivo. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.

## 2. Escopo

### Incluído
- condições por cliente, segmento, pedido, produto, evento e metadata.
- operadores booleanos, numéricos, textuais, temporais e conjunto.
- grupos AND/OR/NOT.
...
### Fora de escopo
- execução de resgate.
- cálculo de desconto monetário.
- segmentação ativa em massa.
- IA para criação automática de regras.
...
3. Ator cria ou consulta recurso de engine de regras e elegibilidade com payload válido.
...
Fluxos alternativos:
1. Ator executa criação de AST usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r011:read`.
- Escrita: `r011:write`.
- Execução de ação crítica: `r011:execute`.
- Administração: `r011:admin`.
- Auditoria/exportação: `r011:audit` ou `r011:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de engine de regras e elegibilidade deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como criação de AST, publicação de versão, avaliação sem efeitos colaterais só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de engine de regras e elegibilidade devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de engine de regras e elegibilidade.  
  Prioridade: Must.  
  Dependências: R004, R005, R006, R022.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como criação de AST, publicação de versão, avaliação sem efeitos colaterais, explicação de falha, rollback de regra.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de engine de regras e elegibilidade devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/motor-regras-elegibilidade` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `RuleSet`: conjunto de regras. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RuleVersion`: versão publicada. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RuleCondition`: condição. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RuleAssignment`: vínculo. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RuleEvaluationResult`: resultado explicado. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/rules`  
  Objetivo: listar recursos de engine de regras e elegibilidade com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r011:read`.  
...
- `POST /api/v1/rules`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r011:write`.  
...
- `GET /api/v1/rules/{id}`  
  ...
  Autenticação exigida: scope `r011:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/rules/{id}`  
  ...
  Autenticação exigida: scope `r011:write`.  
...
- **`rule_set.created`**  
  Quando ocorre: após criação de AST ou mudança relevante no domínio de engine de regras e elegibilidade.  
...
- **`rule_evaluated.failed`**  
  Quando ocorre: após criação de AST ou mudança relevante no domínio de engine de regras e elegibilidade.  
  Payload mínimo: `eventId`, `eventType`, `occurredAt`, `accountId`, `projectId`, `entityId`, `entityType`, `actorId`, `correlationId`, `version`, `metadata`.  
  Deve gerar webhook: sim, quando o projeto tiver assinatura ativa para esse evento.  
  Deve entrar em outbox: sim, obrigatoriamente na mesma transação da mudança.

- **`rule_assignment.changed`**  
  Quando ocorre: após criação de AST ou mudança relevante no domínio de engine de regras e elegibilidade.  
  Payload mínimo: `eventId`, `eventType`, `occurredAt`, `accountId`, `projectId`, `entityId`, `entityType`, `actorId`, `correlationId`, `version`, `metadata`.  
  Deve gerar webhook: sim, quando o projeto tiver assinatura ativa para esse evento.  
  Deve entrar em outbox: sim, obrigatoriamente na mesma transação da mudança.
...
- Ação: criação de AST.  
  ...
  Entidade afetada: recurso principal de engine de regras e elegibilidade.  
...
- Ação: publicação de versão.  
  ...
  correlationId: obrigatório.  
  Severidade: High.
- Ação: avaliação sem efeitos colaterais.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de engine de regras e elegibilidade.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: explicação de falha.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de engine de regras e elegibilidade.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: rollback de regra.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de engine de regras e elegibilidade.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/rules`: listagem principal.
- `/rules/new`: criação.
- `/rules/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/rules/:id/edit`: edição controlada por estado.
- `/rules/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de engine de regras e elegibilidade não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar criação de AST; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r011_requests_total`
- `voucher_system_r011_failures_total`
- `voucher_system_r011_operation_duration_ms`
- `voucher_system_r011_business_events_total`
...
- Custom event `R011.MotorDeRegrasEElegibilidade.OperationCompleted`.
- Custom event `R011.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para engine de regras e elegibilidade.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para engine de regras e elegibilidade estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/MotorRegrasElegibilidade/...`
- `src/VoucherSystem.Application/MotorRegrasElegibilidade/Commands/...`
- `src/VoucherSystem.Application/MotorRegrasElegibilidade/Queries/...`
- `src/VoucherSystem.Api/Endpoints/MotorRegrasElegibilidade/...`
...
- `src/VoucherSystem.Workers/MotorRegrasElegibilidade/...`
- `frontend/src/features/motor-regras-elegibilidade/...`
- `frontend/src/routes/...`
- `docs/requisitos/R011 - MOTOR-REGRAS-ELEGIBILIDADE.md`
- `tests/VoucherSystem.UnitTests/MotorRegrasElegibilidade/...`
- `tests/VoucherSystem.IntegrationTests/MotorRegrasElegibilidade/...`
- `tests/VoucherSystem.ApiTests/MotorRegrasElegibilidade/...`
- `frontend/tests/motor-regras-elegibilidade/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para engine de regras e elegibilidade.  
