...
Este requisito tem como objetivo transformar perfis, eventos e comportamento em audiências reutilizáveis para campanhas, regras, loyalty e distribuições. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.

## 2. Escopo

### Incluído
- segmentos estáticos.
- segmentos dinâmicos passivos e ativos.
- filtros por perfil, metadata, pedidos, eventos e consentimento.
- preview de audiência.
- avaliação em tempo real.
- exportação de membros.
- eventos de entrada e saída.
- versionamento quando em uso crítico.

### Fora de escopo
- modelos preditivos de audiência.
- lookalike externo.
...
3. Ator cria ou consulta recurso de segmentação, audiência e avaliação de pertencimento com payload válido.
...
Fluxos alternativos:
1. Ator executa publicação de segmento usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r005:read`.
- Escrita: `r005:write`.
- Execução de ação crítica: `r005:execute`.
- Administração: `r005:admin`.
- Auditoria/exportação: `r005:audit` ou `r005:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de segmentação, audiência e avaliação de pertencimento deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como publicação de segmento, reavaliação em massa, entrada/saída de membro só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de segmentação, audiência e avaliação de pertencimento devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de segmentação, audiência e avaliação de pertencimento.  
  Prioridade: Must.  
  Dependências: R004, R011.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como publicação de segmento, reavaliação em massa, entrada/saída de membro, exportação de audiência, uso em regra.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de segmentação, audiência e avaliação de pertencimento devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/segmentos-audiencias` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Segment`: definição do segmento. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `SegmentVersion`: versão de regra. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `SegmentMembership`: membro. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `SegmentEvaluation`: execução de avaliação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `AudienceExport`: exportação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/segments`  
  Objetivo: listar recursos de segmentação, audiência e avaliação de pertencimento com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r005:read`.  
...
- `POST /api/v1/segments`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r005:write`.  
...
- `GET /api/v1/segments/{id}`  
  ...
  Autenticação exigida: scope `r005:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/segments/{id}`  
  ...
  Autenticação exigida: scope `r005:write`.  
...
- **`segment.created`**  
  Quando ocorre: após publicação de segmento ou mudança relevante no domínio de segmentação, audiência e avaliação de pertencimento.  
...
- **`segment.member_entered`**  
  Quando ocorre: após publicação de segmento ou mudança relevante no domínio de segmentação, audiência e avaliação de pertencimento.  
  Payload mínimo: `eventId`, `eventType`, `occurredAt`, `accountId`, `projectId`, `entityId`, `entityType`, `actorId`, `correlationId`, `version`, `metadata`.  
  Deve gerar webhook: sim, quando o projeto tiver assinatura ativa para esse evento.  
  Deve entrar em outbox: sim, obrigatoriamente na mesma transação da mudança.

- **`segment.member_exited`**  
  Quando ocorre: após publicação de segmento ou mudança relevante no domínio de segmentação, audiência e avaliação de pertencimento.  
  Payload mínimo: `eventId`, `eventType`, `occurredAt`, `accountId`, `projectId`, `entityId`, `entityType`, `actorId`, `correlationId`, `version`, `metadata`.  
  Deve gerar webhook: sim, quando o projeto tiver assinatura ativa para esse evento.  
  Deve entrar em outbox: sim, obrigatoriamente na mesma transação da mudança.
...
- Ação: publicação de segmento.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de segmentação, audiência e avaliação de pertencimento.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: reavaliação em massa.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de segmentação, audiência e avaliação de pertencimento.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: entrada/saída de membro.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de segmentação, audiência e avaliação de pertencimento.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: exportação de audiência.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de segmentação, audiência e avaliação de pertencimento.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/audiences/segments`: listagem principal.
- `/audiences/segments/new`: criação.
- `/audiences/segments/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/audiences/segments/:id/edit`: edição controlada por estado.
- `/audiences/segments/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de segmentação, audiência e avaliação de pertencimento não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar publicação de segmento; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r005_requests_total`
- `voucher_system_r005_failures_total`
- `voucher_system_r005_operation_duration_ms`
- `voucher_system_r005_business_events_total`
...
- Custom event `R005.SegmentosEAudiencias.OperationCompleted`.
- Custom event `R005.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para segmentação, audiência e avaliação de pertencimento.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para segmentação, audiência e avaliação de pertencimento estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/SegmentosAudiencias/...`
- `src/VoucherSystem.Application/SegmentosAudiencias/Commands/...`
- `src/VoucherSystem.Application/SegmentosAudiencias/Queries/...`
- `src/VoucherSystem.Api/Endpoints/SegmentosAudiencias/...`
...
- `src/VoucherSystem.Workers/SegmentosAudiencias/...`
- `frontend/src/features/segmentos-audiencias/...`
- `frontend/src/routes/...`
- `docs/requisitos/R005 - SEGMENTOS-AUDIENCIAS.md`
- `tests/VoucherSystem.UnitTests/SegmentosAudiencias/...`
- `tests/VoucherSystem.IntegrationTests/SegmentosAudiencias/...`
- `tests/VoucherSystem.ApiTests/SegmentosAudiencias/...`
- `frontend/tests/segmentos-audiencias/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para segmentação, audiência e avaliação de pertencimento.  
