...
Este requisito tem como objetivo administrar valor armazenado com emissão, crédito, débito, estorno e expiração com rastreabilidade financeira. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
3. Ator cria ou consulta recurso de stored value, créditos e ledger monetário com payload válido.
...
Fluxos alternativos:
1. Ator executa emissão usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r015:read`.
- Escrita: `r015:write`.
- Execução de ação crítica: `r015:execute`.
- Administração: `r015:admin`.
- Auditoria/exportação: `r015:audit` ou `r015:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de stored value, créditos e ledger monetário deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como emissão, recarga, débito concorrente só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de stored value, créditos e ledger monetário devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de stored value, créditos e ledger monetário.  
  Prioridade: Must.  
  Dependências: R009, R013.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como emissão, recarga, débito concorrente, estorno, expiração.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de stored value, créditos e ledger monetário devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/gift-cards-creditos-ledger-financeiro` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `GiftCard`: cartão. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `StoredValueAccount`: conta de valor. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `LedgerEntry`: lançamento. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `amount numeric(18,6)`, `currency/points_type text`, `reference_id uuid`, `metadata jsonb`, `created_at timestamptz`, `row_version`.
- `BalanceSnapshot`: saldo reconciliável. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `amount numeric(18,6)`, `currency/points_type text`, `reference_id uuid`, `metadata jsonb`, `created_at timestamptz`, `row_version`.
- `CreditExpirationBucket`: bucket de expiração. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `GiftCardAdjustment`: ajuste. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/gift-cards`  
  Objetivo: listar recursos de stored value, créditos e ledger monetário com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r015:read`.  
...
- `POST /api/v1/gift-cards`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r015:write`.  
...
- `GET /api/v1/gift-cards/{id}`  
  ...
  Autenticação exigida: scope `r015:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/gift-cards/{id}`  
  ...
  Autenticação exigida: scope `r015:write`.  
...
- **`gift_card.issued`**  
  Quando ocorre: após emissão ou mudança relevante no domínio de stored value, créditos e ledger monetário.  
...
- Ação: emissão.  
  ...
  Entidade afetada: recurso principal de stored value, créditos e ledger monetário.  
...
Rotas:
- `/gift-cards`: listagem principal.
- `/gift-cards/new`: criação.
- `/gift-cards/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/gift-cards/:id/edit`: edição controlada por estado.
- `/gift-cards/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de stored value, créditos e ledger monetário não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar emissão; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r015_requests_total`
- `voucher_system_r015_failures_total`
- `voucher_system_r015_operation_duration_ms`
- `voucher_system_r015_business_events_total`
...
- Custom event `R015.GiftCardsCreditosELedgerFinanceiro.OperationCompleted`.
- Custom event `R015.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para stored value, créditos e ledger monetário.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para stored value, créditos e ledger monetário estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/GiftCardsCreditosLedgerFinanceiro/...`
- `src/VoucherSystem.Application/GiftCardsCreditosLedgerFinanceiro/Commands/...`
- `src/VoucherSystem.Application/GiftCardsCreditosLedgerFinanceiro/Queries/...`
- `src/VoucherSystem.Api/Endpoints/GiftCardsCreditosLedgerFinanceiro/...`
...
- `src/VoucherSystem.Workers/GiftCardsCreditosLedgerFinanceiro/...`
- `frontend/src/features/gift-cards-creditos-ledger-financeiro/...`
- `frontend/src/routes/...`
- `docs/requisitos/R015 - GIFT-CARDS-CREDITOS-LEDGER-FINANCEIRO.md`
- `tests/VoucherSystem.UnitTests/GiftCardsCreditosLedgerFinanceiro/...`
- `tests/VoucherSystem.IntegrationTests/GiftCardsCreditosLedgerFinanceiro/...`
- `tests/VoucherSystem.ApiTests/GiftCardsCreditosLedgerFinanceiro/...`
- `frontend/tests/gift-cards-creditos-ledger-financeiro/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para stored value, créditos e ledger monetário.  
