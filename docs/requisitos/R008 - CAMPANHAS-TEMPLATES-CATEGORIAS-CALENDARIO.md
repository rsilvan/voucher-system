...
Este requisito tem como objetivo fornecer a raiz de configuração, governança, publicação, calendário e mensuração dos programas de incentivo. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.

## 2. Escopo

### Incluído
- campanhas draft/scheduled/active/paused/ended/archived.
- tipos coupon, promotion, gift card, loyalty e referral.
- períodos, recorrência, dias e horários válidos.
- categorias, tags e owner.
...
### Fora de escopo
- cálculo de desconto específico.
- resgate transacional.
...
3. Ator cria ou consulta recurso de governança e ciclo de vida de campanhas com payload válido.
...
Fluxos alternativos:
1. Ator executa criação de campanha usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r008:read`.
- Escrita: `r008:write`.
- Execução de ação crítica: `r008:execute`.
- Administração: `r008:admin`.
- Auditoria/exportação: `r008:audit` ou `r008:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de governança e ciclo de vida de campanhas deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como criação de campanha, publicação de versão, ativação/agendamento só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de governança e ciclo de vida de campanhas devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de governança e ciclo de vida de campanhas.  
  Prioridade: Must.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como criação de campanha, publicação de versão, ativação/agendamento, pausa/encerramento, cópia por template.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de governança e ciclo de vida de campanhas devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/campanhas-templates-categorias-calendario` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Campaign`: campanha. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `CampaignVersion`: configuração publicada. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `CampaignTemplate`: template. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `CampaignCategory`: categoria. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `CampaignApproval`: aprovação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `CampaignCalendarEntry`: agenda. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/campaigns`  
  Objetivo: listar recursos de governança e ciclo de vida de campanhas com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r008:read`.  
...
- `POST /api/v1/campaigns`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r008:write`.  
...
- `GET /api/v1/campaigns/{id}`  
  ...
  Autenticação exigida: scope `r008:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/campaigns/{id}`  
  ...
  Autenticação exigida: scope `r008:write`.  
...
- **`campaign.created`**  
  Quando ocorre: após criação de campanha ou mudança relevante no domínio de governança e ciclo de vida de campanhas.  
...
- Ação: criação de campanha.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de governança e ciclo de vida de campanhas.  
...
- Ação: publicação de versão.  
  ...
  correlationId: obrigatório.  
  Severidade: High.
- Ação: ativação/agendamento.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de governança e ciclo de vida de campanhas.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: pausa/encerramento.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de governança e ciclo de vida de campanhas.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: cópia por template.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de governança e ciclo de vida de campanhas.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/campaigns`: listagem principal.
- `/campaigns/new`: criação.
- `/campaigns/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/campaigns/:id/edit`: edição controlada por estado.
- `/campaigns/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de governança e ciclo de vida de campanhas não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar criação de campanha; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r008_requests_total`
- `voucher_system_r008_failures_total`
- `voucher_system_r008_operation_duration_ms`
- `voucher_system_r008_business_events_total`
...
- Custom event `R008.CampanhasTemplatesCategoriasECalendario.OperationCompleted`.
- Custom event `R008.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para governança e ciclo de vida de campanhas.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para governança e ciclo de vida de campanhas estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/CampanhasTemplatesCategoriasCalendario/...`
- `src/VoucherSystem.Application/CampanhasTemplatesCategoriasCalendario/Commands/...`
- `src/VoucherSystem.Application/CampanhasTemplatesCategoriasCalendario/Queries/...`
- `src/VoucherSystem.Api/Endpoints/CampanhasTemplatesCategoriasCalendario/...`
...
- `src/VoucherSystem.Workers/CampanhasTemplatesCategoriasCalendario/...`
- `frontend/src/features/campanhas-templates-categorias-calendario/...`
- `frontend/src/routes/...`
- `docs/requisitos/R008 - CAMPANHAS-TEMPLATES-CATEGORIAS-CALENDARIO.md`
- `tests/VoucherSystem.UnitTests/CampanhasTemplatesCategoriasCalendario/...`
- `tests/VoucherSystem.IntegrationTests/CampanhasTemplatesCategoriasCalendario/...`
- `tests/VoucherSystem.ApiTests/CampanhasTemplatesCategoriasCalendario/...`
- `frontend/tests/campanhas-templates-categorias-calendario/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para governança e ciclo de vida de campanhas.  
