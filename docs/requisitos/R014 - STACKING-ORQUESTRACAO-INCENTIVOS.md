...
Este requisito tem como objetivo controlar como diferentes incentivos são combinados para maximizar conversão sem perder margem, previsibilidade e governança. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.

## 2. Escopo

### Incluído
- aplicação ALL/PARTIAL.
- categorias joint/exclusive.
- limites totais e por categoria.
- ordem solicitada, prioridade e melhor desconto.
- combinação coupon/promotion/gift card/loyalty/referral.
- política para incentivos sem efeito.
- parent redemption.
- explicação dos itens ignorados.

### Fora de escopo
- engine de regra base.
- criação de cada incentivo individual.
- otimização por IA.
- negociação comercial automática.
...
3. Ator cria ou consulta recurso de combinação, prioridade e orquestração de incentivos com payload válido.
...
Fluxos alternativos:
1. Ator executa avaliação de combinação usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r014:read`.
- Escrita: `r014:write`.
- Execução de ação crítica: `r014:execute`.
- Administração: `r014:admin`.
- Auditoria/exportação: `r014:audit` ou `r014:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de combinação, prioridade e orquestração de incentivos deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como avaliação de combinação, priorização, cálculo de melhor desconto só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de combinação, prioridade e orquestração de incentivos devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de combinação, prioridade e orquestração de incentivos.  
  Prioridade: Must.  
  Dependências: R010, R012, R013.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como avaliação de combinação, priorização, cálculo de melhor desconto, resgate agrupado, rollback do conjunto.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de combinação, prioridade e orquestração de incentivos devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/stacking-orquestracao-incentivos` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `StackingPolicy`: política. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `StackingCategory`: categoria. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `StackingEvaluation`: avaliação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `StackingApplication`: aplicação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ParentRedemption`: resgate agrupador. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/stacking`  
  Objetivo: listar recursos de combinação, prioridade e orquestração de incentivos com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r014:read`.  
...
- `POST /api/v1/stacking`  
  Objetivo: criar recurso principal do requisito.  
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r014:write`.  
...
- `GET /api/v1/stacking/{id}`  
  Objetivo: consultar detalhe, timeline resumida e estado atual.  
  Autenticação exigida: scope `r014:read` e acesso ao `account_id/project_id`.  
...
- **`stacking_policy.created`**  
  Quando ocorre: após avaliação de combinação ou mudança relevante no domínio de combinação, prioridade e orquestração de incentivos.  
...
- Ação: avaliação de combinação.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de combinação, prioridade e orquestração de incentivos.  
...
- Ação: priorização.  
  ...
  correlationId: obrigatório.  
  Severidade: High.
- Ação: cálculo de melhor desconto.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de combinação, prioridade e orquestração de incentivos.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: resgate agrupado.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de combinação, prioridade e orquestração de incentivos.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: rollback do conjunto.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de combinação, prioridade e orquestração de incentivos.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/stacking`: listagem principal.
- `/stacking/new`: criação.
- `/stacking/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/stacking/:id/edit`: edição controlada por estado.
- `/stacking/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de combinação, prioridade e orquestração de incentivos não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar avaliação de combinação; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r014_requests_total`
- `voucher_system_r014_failures_total`
- `voucher_system_r014_operation_duration_ms`
- `voucher_system_r014_business_events_total`
...
- Custom event `R014.StackingEOrquestracaoDeIncentivos.OperationCompleted`.
- Custom event `R014.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para combinação, prioridade e orquestração de incentivos.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para combinação, prioridade e orquestração de incentivos estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/StackingOrquestracaoIncentivos/...`
- `src/VoucherSystem.Application/StackingOrquestracaoIncentivos/Commands/...`
- `src/VoucherSystem.Application/StackingOrquestracaoIncentivos/Queries/...`
- `src/VoucherSystem.Api/Endpoints/StackingOrquestracaoIncentivos/...`
...
- `src/VoucherSystem.Workers/StackingOrquestracaoIncentivos/...`
- `frontend/src/features/stacking-orquestracao-incentivos/...`
- `frontend/src/routes/...`
- `docs/requisitos/R014 - STACKING-ORQUESTRACAO-INCENTIVOS.md`
- `tests/VoucherSystem.UnitTests/StackingOrquestracaoIncentivos/...`
- `tests/VoucherSystem.IntegrationTests/StackingOrquestracaoIncentivos/...`
- `tests/VoucherSystem.ApiTests/StackingOrquestracaoIncentivos/...`
- `frontend/tests/stacking-orquestracao-incentivos/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para combinação, prioridade e orquestração de incentivos.  
