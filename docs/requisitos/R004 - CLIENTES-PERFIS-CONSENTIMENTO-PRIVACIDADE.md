...
- anonimização, retenção, exportação e apagamento.
- histórico de incentivos atribuídos.

### Fora de escopo
- CDP completo.
- resolução avançada de identidade por ML.
- enriquecimento externo de dados.
- campanhas e regras, cobertas em outros RNNN.
...
3. Ator cria ou consulta recurso de cliente, PII, consentimento e privacidade com payload válido.
...
Fluxos alternativos:
1. Ator executa upsert de cliente usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r004:read`.
- Escrita: `r004:write`.
- Execução de ação crítica: `r004:execute`.
- Administração: `r004:admin`.
- Auditoria/exportação: `r004:audit` ou `r004:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de cliente, PII, consentimento e privacidade deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como upsert de cliente, merge de perfis, alteração de consentimento só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de cliente, PII, consentimento e privacidade devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de cliente, PII, consentimento e privacidade.  
  Prioridade: Must.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como upsert de cliente, merge de perfis, alteração de consentimento, anonimização, exportação de dados.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de cliente, PII, consentimento e privacidade devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/clientes-perfis-consentimento-privacidade` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Customer`: perfil de cliente. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `CustomerContact`: contatos. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `CustomerAddress`: endereços. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ConsentRecord`: consentimento. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `PrivacyRequest`: solicitação LGPD. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `CustomerMerge`: merge. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/customers`  
  Objetivo: listar recursos de cliente, PII, consentimento e privacidade com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r004:read`.  
...
- `POST /api/v1/customers`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r004:write`.  
...
- `GET /api/v1/customers/{id}`  
  ...
  Autenticação exigida: scope `r004:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/customers/{id}`  
  ...
  Autenticação exigida: scope `r004:write`.  
...
- **`customer.created`**  
  Quando ocorre: após upsert de cliente ou mudança relevante no domínio de cliente, PII, consentimento e privacidade.  
...
Rotas:
- `/customers`: listagem principal.
- `/customers/new`: criação.
- `/customers/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/customers/:id/edit`: edição controlada por estado.
- `/customers/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de cliente, PII, consentimento e privacidade não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar upsert de cliente; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r004_requests_total`
- `voucher_system_r004_failures_total`
- `voucher_system_r004_operation_duration_ms`
- `voucher_system_r004_business_events_total`
...
- Custom event `R004.ClientesPerfisConsentimentoEPrivacidade.OperationCompleted`.
- Custom event `R004.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para cliente, PII, consentimento e privacidade.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para cliente, PII, consentimento e privacidade estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/ClientesPerfisConsentimentoPrivacidade/...`
- `src/VoucherSystem.Application/ClientesPerfisConsentimentoPrivacidade/Commands/...`
- `src/VoucherSystem.Application/ClientesPerfisConsentimentoPrivacidade/Queries/...`
- `src/VoucherSystem.Api/Endpoints/ClientesPerfisConsentimentoPrivacidade/...`
...
- `src/VoucherSystem.Workers/ClientesPerfisConsentimentoPrivacidade/...`
- `frontend/src/features/clientes-perfis-consentimento-privacidade/...`
- `frontend/src/routes/...`
- `docs/requisitos/R004 - CLIENTES-PERFIS-CONSENTIMENTO-PRIVACIDADE.md`
- `tests/VoucherSystem.UnitTests/ClientesPerfisConsentimentoPrivacidade/...`
- `tests/VoucherSystem.IntegrationTests/ClientesPerfisConsentimentoPrivacidade/...`
- `tests/VoucherSystem.ApiTests/ClientesPerfisConsentimentoPrivacidade/...`
- `frontend/tests/clientes-perfis-consentimento-privacidade/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para cliente, PII, consentimento e privacidade.  
