...
Este requisito tem como objetivo proteger clientes, dados e operações e fornecer evidências para auditoria, contratos e resposta a incidentes. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
- resposta a incidentes e rotação.

### Fora de escopo
- certificação formal ISO/SOC no MVP.
- SIEM enterprise integrado.
- DLP avançado.
- MFA obrigatório desde o primeiro release.
...
3. Ator cria ou consulta recurso de segurança, privacidade e conformidade com payload válido.
...
Fluxos alternativos:
1. Ator executa verificação de permissão server-side usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r028:read`.
- Escrita: `r028:write`.
- Execução de ação crítica: `r028:execute`.
- Administração: `r028:admin`.
- Auditoria/exportação: `r028:audit` ou `r028:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de segurança, privacidade e conformidade deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como verificação de permissão server-side, rotação de segredo, sanitização de log só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de segurança, privacidade e conformidade devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de segurança, privacidade e conformidade.  
  Prioridade: Must.  
  Dependências: R001, R003, R027.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como verificação de permissão server-side, rotação de segredo, sanitização de log, apagamento/anonimização, scan no pipeline.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de segurança, privacidade e conformidade devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/seguranca-privacidade-compliance` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `SecurityPolicy`: política. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `SecretReference`: referência ao Key Vault. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `AccessReview`: revisão. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `SecurityIncident`: incidente. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `PrivacyPolicy`: política de privacidade. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `DataRetentionRule`: retenção. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/security`  
  Objetivo: listar recursos de segurança, privacidade e conformidade com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r028:read`.  
...
- `POST /api/v1/security`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r028:write`.  
...
- `GET /api/v1/security/{id}`  
  ...
  Autenticação exigida: scope `r028:read` e acesso ao `account_id/project_id`.  
...
- **`security_policy.updated`**  
  Quando ocorre: após verificação de permissão server-side ou mudança relevante no domínio de segurança, privacidade e conformidade.  
...
- Ação: verificação de permissão server-side.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de segurança, privacidade e conformidade.  
...
- Ação: rotação de segredo.  
  ...
  correlationId: obrigatório.  
  Severidade: High.
- Ação: sanitização de log.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de segurança, privacidade e conformidade.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: apagamento/anonimização.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de segurança, privacidade e conformidade.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: scan no pipeline.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de segurança, privacidade e conformidade.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/security`: listagem principal.
- `/security/new`: criação.
- `/security/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/security/:id/edit`: edição controlada por estado.
- `/security/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de segurança, privacidade e conformidade não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar verificação de permissão server-side; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r028_requests_total`
- `voucher_system_r028_failures_total`
- `voucher_system_r028_operation_duration_ms`
- `voucher_system_r028_business_events_total`
...
- Custom event `R028.SegurancaPrivacidadeECompliance.OperationCompleted`.
- Custom event `R028.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para segurança, privacidade e conformidade.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para segurança, privacidade e conformidade estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/SegurancaPrivacidadeCompliance/...`
- `src/VoucherSystem.Application/SegurancaPrivacidadeCompliance/Commands/...`
- `src/VoucherSystem.Application/SegurancaPrivacidadeCompliance/Queries/...`
- `src/VoucherSystem.Api/Endpoints/SegurancaPrivacidadeCompliance/...`
...
- `src/VoucherSystem.Workers/SegurancaPrivacidadeCompliance/...`
- `frontend/src/features/seguranca-privacidade-compliance/...`
- `frontend/src/routes/...`
- `docs/requisitos/R028 - SEGURANCA-PRIVACIDADE-COMPLIANCE.md`
- `tests/VoucherSystem.UnitTests/SegurancaPrivacidadeCompliance/...`
- `tests/VoucherSystem.IntegrationTests/SegurancaPrivacidadeCompliance/...`
- `tests/VoucherSystem.ApiTests/SegurancaPrivacidadeCompliance/...`
- `frontend/tests/seguranca-privacidade-compliance/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para segurança, privacidade e conformidade.  
