...
Este requisito tem como objetivo dar visibilidade sobre desempenho, custo, operação, alterações e resultados assíncronos. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
- receita, desconto, breakage e custo.
...
- notificações push mobile nativas.
...
3. Ator cria ou consulta recurso de insights, auditoria, exportação e notificação com payload válido.
...
Fluxos alternativos:
1. Ator executa registro de auditoria usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r023:read`.
- Escrita: `r023:write`.
- Execução de ação crítica: `r023:execute`.
- Administração: `r023:admin`.
- Auditoria/exportação: `r023:audit` ou `r023:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de insights, auditoria, exportação e notificação deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como registro de auditoria, agregação assíncrona, exportação segura só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de insights, auditoria, exportação e notificação devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de insights, auditoria, exportação e notificação.  
  Prioridade: Must.  
  Dependências: Todos os domínios operacionais.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como registro de auditoria, agregação assíncrona, exportação segura, geração de notificação, consulta com permissão.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de insights, auditoria, exportação e notificação devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/analytics-auditoria-exportacoes-notificacoes` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `AnalyticsMetric`: métrica agregada. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `AuditLog`: auditoria. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ExportJob`: exportação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `Notification`: notificação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `TimelineEntry`: timeline. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ReportFile`: arquivo. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/analytics`  
  Objetivo: listar recursos de insights, auditoria, exportação e notificação com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r023:read`.  
...
- `POST /api/v1/analytics`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r023:write`.  
...
- `GET /api/v1/analytics/{id}`  
  ...
  Autenticação exigida: scope `r023:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/analytics/{id}`  
  ...
  Autenticação exigida: scope `r023:write`.  
...
- **`audit_log.created`**  
  Quando ocorre: após registro de auditoria ou mudança relevante no domínio de insights, auditoria, exportação e notificação.  
...
- Ação: registro de auditoria.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de insights, auditoria, exportação e notificação.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: agregação assíncrona.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de insights, auditoria, exportação e notificação.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: exportação segura.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de insights, auditoria, exportação e notificação.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: geração de notificação.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de insights, auditoria, exportação e notificação.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: consulta com permissão.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de insights, auditoria, exportação e notificação.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/analytics`: listagem principal.
- `/analytics/new`: criação.
- `/analytics/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/analytics/:id/edit`: edição controlada por estado.
- `/analytics/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de insights, auditoria, exportação e notificação não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar registro de auditoria; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r023_requests_total`
- `voucher_system_r023_failures_total`
- `voucher_system_r023_operation_duration_ms`
- `voucher_system_r023_business_events_total`
...
- Custom event `R023.AnalyticsAuditoriaExportacoesENotificacoes.OperationCompleted`.
- Custom event `R023.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para insights, auditoria, exportação e notificação.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para insights, auditoria, exportação e notificação estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/AnalyticsAuditoriaExportacoesNotificacoes/...`
- `src/VoucherSystem.Application/AnalyticsAuditoriaExportacoesNotificacoes/Commands/...`
- `src/VoucherSystem.Application/AnalyticsAuditoriaExportacoesNotificacoes/Queries/...`
- `src/VoucherSystem.Api/Endpoints/AnalyticsAuditoriaExportacoesNotificacoes/...`
...
- `src/VoucherSystem.Workers/AnalyticsAuditoriaExportacoesNotificacoes/...`
- `frontend/src/features/analytics-auditoria-exportacoes-notificacoes/...`
- `frontend/src/routes/...`
- `docs/requisitos/R023 - ANALYTICS-AUDITORIA-EXPORTACOES-NOTIFICACOES.md`
- `tests/VoucherSystem.UnitTests/AnalyticsAuditoriaExportacoesNotificacoes/...`
- `tests/VoucherSystem.IntegrationTests/AnalyticsAuditoriaExportacoesNotificacoes/...`
- `tests/VoucherSystem.ApiTests/AnalyticsAuditoriaExportacoesNotificacoes/...`
- `frontend/tests/analytics-auditoria-exportacoes-notificacoes/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para insights, auditoria, exportação e notificação.  
