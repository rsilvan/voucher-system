...
Este requisito tem como objetivo descobrir incentivos aplicáveis, validar redeemables selecionados e reservar temporariamente capacidade durante o checkout. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
- respostas applicable/inapplicable/skipped.
- cálculo do pedido resultante.
- motivos de falha.
- validation sessions com lock, TTL, overwrite e release.
- surface client-side restrita.
- tracking ID e auditoria.
...
3. Ator cria ou consulta recurso de qualification, validation e reservation locks com payload válido.
...
Fluxos alternativos:
1. Ator executa qualification usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r012:read`.
- Escrita: `r012:write`.
- Execução de ação crítica: `r012:execute`.
- Administração: `r012:admin`.
- Auditoria/exportação: `r012:audit` ou `r012:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de qualification, validation e reservation locks deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como qualification, validação multi-redeemable, criação de session lock só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de qualification, validation e reservation locks devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de qualification, validation e reservation locks.  
  Prioridade: Must.  
  Dependências: R009, R010, R011.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como qualification, validação multi-redeemable, criação de session lock, expiração de sessão, release/overwrite.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de qualification, validation e reservation locks devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/qualificacao-validacao-sessoes` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `ValidationRequest`: requisição. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ValidationResult`: resultado. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ValidationSession`: sessão/lock. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `QualifiedIncentive`: incentivo qualificado. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ValidationFailureReason`: motivo. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/validations`  
  Objetivo: listar recursos de qualification, validation e reservation locks com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r012:read`.  
...
- `POST /api/v1/validations`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r012:write`.  
...
- `GET /api/v1/validations/{id}`  
  ...
  Autenticação exigida: scope `r012:read` e acesso ao `account_id/project_id`.  
...
- **`validation.requested`**  
  Quando ocorre: após qualification ou mudança relevante no domínio de qualification, validation e reservation locks.  
...
- **`validation_session.locked`**  
  Quando ocorre: após qualification ou mudança relevante no domínio de qualification, validation e reservation locks.  
  Payload mínimo: `eventId`, `eventType`, `occurredAt`, `accountId`, `projectId`, `entityId`, `entityType`, `actorId`, `correlationId`, `version`, `metadata`.  
  Deve gerar webhook: sim, quando o projeto tiver assinatura ativa para esse evento.  
  Deve entrar em outbox: sim, obrigatoriamente na mesma transação da mudança.

- **`validation_session.released`**  
  Quando ocorre: após qualification ou mudança relevante no domínio de qualification, validation e reservation locks.  
  Payload mínimo: `eventId`, `eventType`, `occurredAt`, `accountId`, `projectId`, `entityId`, `entityType`, `actorId`, `correlationId`, `version`, `metadata`.  
  Deve gerar webhook: sim, quando o projeto tiver assinatura ativa para esse evento.  
  Deve entrar em outbox: sim, obrigatoriamente na mesma transação da mudança.
...
- Ação: qualification.  
  ...
  Entidade afetada: recurso principal de qualification, validation e reservation locks.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: validação multi-redeemable.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de qualification, validation e reservation locks.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: criação de session lock.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de qualification, validation e reservation locks.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: expiração de sessão.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de qualification, validation e reservation locks.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: release/overwrite.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de qualification, validation e reservation locks.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/checkout-simulator`: listagem principal.
- `/checkout-simulator/new`: criação.
- `/checkout-simulator/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/checkout-simulator/:id/edit`: edição controlada por estado.
- `/checkout-simulator/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de qualification, validation e reservation locks não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar qualification; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r012_requests_total`
- `voucher_system_r012_failures_total`
- `voucher_system_r012_operation_duration_ms`
- `voucher_system_r012_business_events_total`
...
- Custom event `R012.QualificacaoValidacaoESessoes.OperationCompleted`.
- Custom event `R012.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para qualification, validation e reservation locks.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para qualification, validation e reservation locks estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/QualificacaoValidacaoSessoes/...`
- `src/VoucherSystem.Application/QualificacaoValidacaoSessoes/Commands/...`
- `src/VoucherSystem.Application/QualificacaoValidacaoSessoes/Queries/...`
- `src/VoucherSystem.Api/Endpoints/QualificacaoValidacaoSessoes/...`
...
- `src/VoucherSystem.Workers/QualificacaoValidacaoSessoes/...`
- `frontend/src/features/qualificacao-validacao-sessoes/...`
- `frontend/src/routes/...`
- `docs/requisitos/R012 - QUALIFICACAO-VALIDACAO-SESSOES.md`
- `tests/VoucherSystem.UnitTests/QualificacaoValidacaoSessoes/...`
- `tests/VoucherSystem.IntegrationTests/QualificacaoValidacaoSessoes/...`
- `tests/VoucherSystem.ApiTests/QualificacaoValidacaoSessoes/...`
- `frontend/tests/qualificacao-validacao-sessoes/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para qualification, validation e reservation locks.  
