...
Este requisito tem como objetivo criar e administrar códigos únicos, genéricos ou stand-alone usados como instrumentos de incentivo. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
- ledger financeiro de gift card.
- loyalty points.
...
3. Ator cria ou consulta recurso de vouchers, códigos e lotes de geração com payload válido.
...
Fluxos alternativos:
1. Ator executa geração de códigos usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r009:read`.
- Escrita: `r009:write`.
- Execução de ação crítica: `r009:execute`.
- Administração: `r009:admin`.
- Auditoria/exportação: `r009:audit` ou `r009:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de vouchers, códigos e lotes de geração deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como geração de códigos, importação, ativação/desativação só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de vouchers, códigos e lotes de geração devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de vouchers, códigos e lotes de geração.  
  Prioridade: Must.  
  Dependências: R008.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como geração de códigos, importação, ativação/desativação, busca por código, atribuição de holder.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de vouchers, códigos e lotes de geração devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/vouchers-codigos-ciclo-vida` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Voucher`: código/redeemable. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `VoucherBatch`: lote. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `VoucherPattern`: padrão de geração. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `VoucherLifecycleEvent`: histórico. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `VoucherHolder`: titular. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `VoucherLimit`: limite. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/vouchers`  
  Objetivo: listar recursos de vouchers, códigos e lotes de geração com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r009:read`.  
...
- `POST /api/v1/vouchers`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r009:write`.  
...
- `GET /api/v1/vouchers/{id}`  
  ...
  Autenticação exigida: scope `r009:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/vouchers/{id}`  
  ...
  Autenticação exigida: scope `r009:write`.  
...
- **`voucher.generated`**  
  Quando ocorre: após geração de códigos ou mudança relevante no domínio de vouchers, códigos e lotes de geração.  
...
- Ação: geração de códigos.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de vouchers, códigos e lotes de geração.  
...
- Ação: importação.  
  ...
  correlationId: obrigatório.  
  Severidade: High.
- Ação: ativação/desativação.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de vouchers, códigos e lotes de geração.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: busca por código.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de vouchers, códigos e lotes de geração.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: atribuição de holder.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de vouchers, códigos e lotes de geração.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/vouchers`: listagem principal.
- `/vouchers/new`: criação.
- `/vouchers/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/vouchers/:id/edit`: edição controlada por estado.
- `/vouchers/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de vouchers, códigos e lotes de geração não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar geração de códigos; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r009_requests_total`
- `voucher_system_r009_failures_total`
- `voucher_system_r009_operation_duration_ms`
- `voucher_system_r009_business_events_total`
...
- Custom event `R009.VouchersCodigosECicloDeVida.OperationCompleted`.
- Custom event `R009.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para vouchers, códigos e lotes de geração.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para vouchers, códigos e lotes de geração estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/VouchersCodigosCicloVida/...`
- `src/VoucherSystem.Application/VouchersCodigosCicloVida/Commands/...`
- `src/VoucherSystem.Application/VouchersCodigosCicloVida/Queries/...`
- `src/VoucherSystem.Api/Endpoints/VouchersCodigosCicloVida/...`
...
- `src/VoucherSystem.Workers/VouchersCodigosCicloVida/...`
- `frontend/src/features/vouchers-codigos-ciclo-vida/...`
- `frontend/src/routes/...`
- `docs/requisitos/R009 - VOUCHERS-CODIGOS-CICLO-VIDA.md`
- `tests/VoucherSystem.UnitTests/VouchersCodigosCicloVida/...`
- `tests/VoucherSystem.IntegrationTests/VouchersCodigosCicloVida/...`
- `tests/VoucherSystem.ApiTests/VouchersCodigosCicloVida/...`
- `frontend/tests/vouchers-codigos-ciclo-vida/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para vouchers, códigos e lotes de geração.  
