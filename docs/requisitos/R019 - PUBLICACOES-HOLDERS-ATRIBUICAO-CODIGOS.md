...
Este requisito tem como objetivo controlar a atribuição de vouchers, loyalty cards e referral codes a clientes com consistência e rastreabilidade. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.

## 2. Escopo

### Incluído
- seleção de código disponível.
- publicação para holder.
...
- histórico e eventos.
- publicação client-side restrita.

### Fora de escopo
- envio de mensagem.
- resgate final.
- geração em massa avançada.
...
3. Ator cria ou consulta recurso de atribuição de redeemables a holders com payload válido.
...
Fluxos alternativos:
1. Ator executa seleção atômica de código usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r019:read`.
- Escrita: `r019:write`.
- Execução de ação crítica: `r019:execute`.
- Administração: `r019:admin`.
- Auditoria/exportação: `r019:audit` ou `r019:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de atribuição de redeemables a holders deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como seleção atômica de código, atribuição a holder, retry idempotente só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de atribuição de redeemables a holders devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de atribuição de redeemables a holders.  
  Prioridade: Must.  
  Dependências: R004, R009.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como seleção atômica de código, atribuição a holder, retry idempotente, unpublish, listagem por cliente.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de atribuição de redeemables a holders devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/publicacoes-holders-atribuicao-codigos` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Publication`: publicação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `Holder`: titular. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `PublicationPolicy`: política. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RedeemableAssignment`: atribuição. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `PublicationAttempt`: tentativa. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/publications`  
  Objetivo: listar recursos de atribuição de redeemables a holders com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r019:read`.  
...
- `POST /api/v1/publications`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r019:write`.  
...
- `GET /api/v1/publications/{id}`  
  ...
  Autenticação exigida: scope `r019:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/publications/{id}`  
  ...
  Autenticação exigida: scope `r019:write`.  
...
- **`publication.created`**  
  Quando ocorre: após seleção atômica de código ou mudança relevante no domínio de atribuição de redeemables a holders.  
...
Rotas:
- `/publications`: listagem principal.
- `/publications/new`: criação.
- `/publications/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/publications/:id/edit`: edição controlada por estado.
- `/publications/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de atribuição de redeemables a holders não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar seleção atômica de código; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r019_requests_total`
- `voucher_system_r019_failures_total`
- `voucher_system_r019_operation_duration_ms`
- `voucher_system_r019_business_events_total`
...
- Custom event `R019.PublicacoesHoldersEAtribuicaoDeCodigos.OperationCompleted`.
- Custom event `R019.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para atribuição de redeemables a holders.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para atribuição de redeemables a holders estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/PublicacoesHoldersAtribuicaoCodigos/...`
- `src/VoucherSystem.Application/PublicacoesHoldersAtribuicaoCodigos/Commands/...`
- `src/VoucherSystem.Application/PublicacoesHoldersAtribuicaoCodigos/Queries/...`
- `src/VoucherSystem.Api/Endpoints/PublicacoesHoldersAtribuicaoCodigos/...`
...
- `src/VoucherSystem.Workers/PublicacoesHoldersAtribuicaoCodigos/...`
- `frontend/src/features/publicacoes-holders-atribuicao-codigos/...`
- `frontend/src/routes/...`
- `docs/requisitos/R019 - PUBLICACOES-HOLDERS-ATRIBUICAO-CODIGOS.md`
- `tests/VoucherSystem.UnitTests/PublicacoesHoldersAtribuicaoCodigos/...`
- `tests/VoucherSystem.IntegrationTests/PublicacoesHoldersAtribuicaoCodigos/...`
- `tests/VoucherSystem.ApiTests/PublicacoesHoldersAtribuicaoCodigos/...`
- `frontend/tests/publicacoes-holders-atribuicao-codigos/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para atribuição de redeemables a holders.  
