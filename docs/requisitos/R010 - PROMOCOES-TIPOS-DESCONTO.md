...
Este requisito tem como objetivo configurar descontos automáticos ou acionados por código para estratégias comerciais determinísticas e auditáveis. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
- arredondamento e rateio entre itens.

### Fora de escopo
- resgate final transacional.
- gift card e saldo armazenado.
- loyalty/rewards.
- otimização automática de margem.
...
3. Ator cria ou consulta recurso de promoções, descontos e cálculo comercial com payload válido.
...
Fluxos alternativos:
1. Ator executa definição de desconto usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r010:read`.
- Escrita: `r010:write`.
- Execução de ação crítica: `r010:execute`.
- Administração: `r010:admin`.
- Auditoria/exportação: `r010:audit` ou `r010:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de promoções, descontos e cálculo comercial deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como definição de desconto, preview de pedido, publicação de promoção só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de promoções, descontos e cálculo comercial devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de promoções, descontos e cálculo comercial.  
  Prioridade: Must.  
  Dependências: R006, R008.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como definição de desconto, preview de pedido, publicação de promoção, cálculo de desconto, validação de moeda/arredondamento.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de promoções, descontos e cálculo comercial devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/promocoes-tipos-desconto` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Promotion`: promoção. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `DiscountDefinition`: definição de desconto. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `DiscountTier`: faixa. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `DiscountApplication`: aplicação calculada. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `PromotionPreview`: simulação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/promotions`  
  Objetivo: listar recursos de promoções, descontos e cálculo comercial com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r010:read`.  
...
- `POST /api/v1/promotions`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r010:write`.  
...
- `GET /api/v1/promotions/{id}`  
  ...
  Autenticação exigida: scope `r010:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/promotions/{id}`  
  ...
  Autenticação exigida: scope `r010:write`.  
...
- **`promotion.created`**  
  Quando ocorre: após definição de desconto ou mudança relevante no domínio de promoções, descontos e cálculo comercial.  
...
- Ação: definição de desconto.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de promoções, descontos e cálculo comercial.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: preview de pedido.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de promoções, descontos e cálculo comercial.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: publicação de promoção.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de promoções, descontos e cálculo comercial.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: cálculo de desconto.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de promoções, descontos e cálculo comercial.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: validação de moeda/arredondamento.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de promoções, descontos e cálculo comercial.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/promotions`: listagem principal.
- `/promotions/new`: criação.
- `/promotions/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/promotions/:id/edit`: edição controlada por estado.
- `/promotions/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de promoções, descontos e cálculo comercial não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar definição de desconto; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r010_requests_total`
- `voucher_system_r010_failures_total`
- `voucher_system_r010_operation_duration_ms`
- `voucher_system_r010_business_events_total`
...
- Custom event `R010.PromocoesETiposDeDesconto.OperationCompleted`.
- Custom event `R010.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para promoções, descontos e cálculo comercial.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para promoções, descontos e cálculo comercial estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/PromocoesTiposDesconto/...`
- `src/VoucherSystem.Application/PromocoesTiposDesconto/Commands/...`
- `src/VoucherSystem.Application/PromocoesTiposDesconto/Queries/...`
- `src/VoucherSystem.Api/Endpoints/PromocoesTiposDesconto/...`
...
- `src/VoucherSystem.Workers/PromocoesTiposDesconto/...`
- `frontend/src/features/promocoes-tipos-desconto/...`
- `frontend/src/routes/...`
- `docs/requisitos/R010 - PROMOCOES-TIPOS-DESCONTO.md`
- `tests/VoucherSystem.UnitTests/PromocoesTiposDesconto/...`
- `tests/VoucherSystem.IntegrationTests/PromocoesTiposDesconto/...`
- `tests/VoucherSystem.ApiTests/PromocoesTiposDesconto/...`
- `frontend/tests/promocoes-tipos-desconto/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para promoções, descontos e cálculo comercial.  
