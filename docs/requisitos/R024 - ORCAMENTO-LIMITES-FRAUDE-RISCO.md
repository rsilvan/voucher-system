...
Este requisito tem como objetivo proteger margem, orçamento e integridade dos programas contra erro de configuração e abuso. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
3. Ator cria ou consulta recurso de budget, limits, antifraude e risco com payload válido.
...
Fluxos alternativos:
1. Ator executa checar budget usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r024:read`.
- Escrita: `r024:write`.
- Execução de ação crítica: `r024:execute`.
- Administração: `r024:admin`.
- Auditoria/exportação: `r024:audit` ou `r024:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de budget, limits, antifraude e risco deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como checar budget, incrementar contador transacional, bloquear tentativa só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de budget, limits, antifraude e risco devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de budget, limits, antifraude e risco.  
  Prioridade: Must.  
  Dependências: R011, R013, R023.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como checar budget, incrementar contador transacional, bloquear tentativa, acionar kill switch, override autorizado.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de budget, limits, antifraude e risco devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/orcamento-limites-fraude-risco` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Budget`: orçamento. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `amount numeric(18,6)`, `currency/points_type text`, `reference_id uuid`, `metadata jsonb`, `created_at timestamptz`, `row_version`.
- `LimitPolicy`: política de limite. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RiskRule`: regra de risco. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RiskDecision`: decisão. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `DenyListEntry`: bloqueio. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `RiskAlert`: alerta. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `KillSwitch`: interrupção. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/risk`  
  Objetivo: listar recursos de budget, limits, antifraude e risco com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r024:read`.  
...
- `POST /api/v1/risk`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r024:write`.  
...
- `GET /api/v1/risk/{id}`  
  ...
  Autenticação exigida: scope `r024:read` e acesso ao `account_id/project_id`.  
...
- **`budget.threshold_reached`**  
  Quando ocorre: após checar budget ou mudança relevante no domínio de budget, limits, antifraude e risco.  
...
- Ação: checar budget.  
  ...
  Entidade afetada: recurso principal de budget, limits, antifraude e risco.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: incrementar contador transacional.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de budget, limits, antifraude e risco.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: bloquear tentativa.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de budget, limits, antifraude e risco.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: acionar kill switch.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de budget, limits, antifraude e risco.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: override autorizado.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de budget, limits, antifraude e risco.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/risk`: listagem principal.
- `/risk/new`: criação.
- `/risk/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/risk/:id/edit`: edição controlada por estado.
- `/risk/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de budget, limits, antifraude e risco não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar checar budget; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r024_requests_total`
- `voucher_system_r024_failures_total`
- `voucher_system_r024_operation_duration_ms`
- `voucher_system_r024_business_events_total`
...
- Custom event `R024.OrcamentoLimitesFraudeEGestaoDeRisco.OperationCompleted`.
- Custom event `R024.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para budget, limits, antifraude e risco.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para budget, limits, antifraude e risco estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/OrcamentoLimitesFraudeRisco/...`
- `src/VoucherSystem.Application/OrcamentoLimitesFraudeRisco/Commands/...`
- `src/VoucherSystem.Application/OrcamentoLimitesFraudeRisco/Queries/...`
- `src/VoucherSystem.Api/Endpoints/OrcamentoLimitesFraudeRisco/...`
...
- `src/VoucherSystem.Workers/OrcamentoLimitesFraudeRisco/...`
- `frontend/src/features/orcamento-limites-fraude-risco/...`
- `frontend/src/routes/...`
- `docs/requisitos/R024 - ORCAMENTO-LIMITES-FRAUDE-RISCO.md`
- `tests/VoucherSystem.UnitTests/OrcamentoLimitesFraudeRisco/...`
- `tests/VoucherSystem.IntegrationTests/OrcamentoLimitesFraudeRisco/...`
- `tests/VoucherSystem.ApiTests/OrcamentoLimitesFraudeRisco/...`
- `frontend/tests/orcamento-limites-fraude-risco/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para budget, limits, antifraude e risco.  
