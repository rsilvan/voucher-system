...
Este requisito tem como objetivo operar grandes volumes sem bloquear o portal ou exigir integrações registro a registro. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
- progresso e sucesso parcial.
- relatório de erros.
- geração/atualização em massa.
...
- ETL visual completo.
- data warehouse.
- transformações arbitrárias em código.
- streaming contínuo.
...
3. Ator cria ou consulta recurso de operações em massa e ações assíncronas com payload válido.
...
Fluxos alternativos:
1. Ator executa upload de arquivo usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r025:read`.
- Escrita: `r025:write`.
- Execução de ação crítica: `r025:execute`.
- Administração: `r025:admin`.
- Auditoria/exportação: `r025:audit` ou `r025:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de operações em massa e ações assíncronas deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como upload de arquivo, dry-run, processamento em chunks só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de operações em massa e ações assíncronas devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de operações em massa e ações assíncronas.  
  Prioridade: Must.  
  Dependências: R003, R021.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como upload de arquivo, dry-run, processamento em chunks, retomada idempotente, cancelamento.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de operações em massa e ações assíncronas devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/imports-bulk-operations-async-actions` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `AsyncAction`: ação assíncrona. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ImportJob`: job de importação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ImportFile`: arquivo. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ImportRowResult`: resultado de linha. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `BulkOperation`: operação em massa. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `JobLease`: lease. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/async-actions`  
  Objetivo: listar recursos de operações em massa e ações assíncronas com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r025:read`.  
...
- `POST /api/v1/async-actions`  
  Objetivo: criar recurso principal do requisito.  
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r025:write`.  
...
- `GET /api/v1/async-actions/{id}`  
  Objetivo: consultar detalhe, timeline resumida e estado atual.  
  Autenticação exigida: scope `r025:read` e acesso ao `account_id/project_id`.  
...
- **`import.requested`**  
  Quando ocorre: após upload de arquivo ou mudança relevante no domínio de operações em massa e ações assíncronas.  
...
- Ação: upload de arquivo.  
  ...
  Entidade afetada: recurso principal de operações em massa e ações assíncronas.  
...
- Ação: dry-run.  
  ...
  correlationId: obrigatório.  
  Severidade: High.
- Ação: processamento em chunks.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de operações em massa e ações assíncronas.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: retomada idempotente.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de operações em massa e ações assíncronas.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/imports`: listagem principal.
- `/imports/new`: criação.
- `/imports/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/imports/:id/edit`: edição controlada por estado.
- `/imports/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de operações em massa e ações assíncronas não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar upload de arquivo; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r025_requests_total`
- `voucher_system_r025_failures_total`
- `voucher_system_r025_operation_duration_ms`
- `voucher_system_r025_business_events_total`
...
- Custom event `R025.ImportsBulkOperationsEAsyncActions.OperationCompleted`.
- Custom event `R025.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para operações em massa e ações assíncronas.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para operações em massa e ações assíncronas estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/ImportsBulkOperationsAsyncActions/...`
- `src/VoucherSystem.Application/ImportsBulkOperationsAsyncActions/Commands/...`
- `src/VoucherSystem.Application/ImportsBulkOperationsAsyncActions/Queries/...`
- `src/VoucherSystem.Api/Endpoints/ImportsBulkOperationsAsyncActions/...`
...
- `src/VoucherSystem.Workers/ImportsBulkOperationsAsyncActions/...`
- `frontend/src/features/imports-bulk-operations-async-actions/...`
- `frontend/src/routes/...`
- `docs/requisitos/R025 - IMPORTS-BULK-OPERATIONS-ASYNC-ACTIONS.md`
- `tests/VoucherSystem.UnitTests/ImportsBulkOperationsAsyncActions/...`
- `tests/VoucherSystem.IntegrationTests/ImportsBulkOperationsAsyncActions/...`
- `tests/VoucherSystem.ApiTests/ImportsBulkOperationsAsyncActions/...`
- `frontend/tests/imports-bulk-operations-async-actions/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para operações em massa e ações assíncronas.  
