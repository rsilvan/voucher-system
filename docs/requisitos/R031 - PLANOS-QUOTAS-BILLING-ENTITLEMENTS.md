...
Este requisito tem como objetivo transformar capacidades da plataforma em ofertas comerciais controláveis, mensuráveis e aplicáveis tecnicamente. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
### Fora de escopo
- processador de pagamentos completo.
- emissão fiscal.
- portal financeiro avançado.
...
3. Ator cria ou consulta recurso de planos comerciais, quotas e habilitação de funcionalidades com payload válido.
...
Fluxos alternativos:
1. Ator executa checagem de entitlement usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r031:read`.
- Escrita: `r031:write`.
- Execução de ação crítica: `r031:execute`.
- Administração: `r031:admin`.
- Auditoria/exportação: `r031:audit` ou `r031:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de planos comerciais, quotas e habilitação de funcionalidades deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como checagem de entitlement, medição idempotente de uso, upgrade/downgrade só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de planos comerciais, quotas e habilitação de funcionalidades devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de planos comerciais, quotas e habilitação de funcionalidades.  
  Prioridade: Must.  
  Dependências: R001, R023.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como checagem de entitlement, medição idempotente de uso, upgrade/downgrade, suspensão, alerta de quota.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de planos comerciais, quotas e habilitação de funcionalidades devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/planos-quotas-billing-entitlements` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Plan`: plano. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `Entitlement`: habilitação. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `Quota`: quota. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `UsageRecord`: uso. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `Subscription`: assinatura. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `BillingEvent`: evento de cobrança. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `GracePeriod`: carência. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/billing`  
  Objetivo: listar recursos de planos comerciais, quotas e habilitação de funcionalidades com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r031:read`.  
...
- `POST /api/v1/billing`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r031:write`.  
...
- `GET /api/v1/billing/{id}`  
  ...
  Autenticação exigida: scope `r031:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/billing/{id}`  
  ...
  Autenticação exigida: scope `r031:write`.  
...
- **`subscription.started`**  
  Quando ocorre: após checagem de entitlement ou mudança relevante no domínio de planos comerciais, quotas e habilitação de funcionalidades.  
...
- Ação: checagem de entitlement.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de planos comerciais, quotas e habilitação de funcionalidades.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: medição idempotente de uso.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de planos comerciais, quotas e habilitação de funcionalidades.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: upgrade/downgrade.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de planos comerciais, quotas e habilitação de funcionalidades.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/billing`: listagem principal.
- `/billing/new`: criação.
- `/billing/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/billing/:id/edit`: edição controlada por estado.
- `/billing/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de planos comerciais, quotas e habilitação de funcionalidades não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar checagem de entitlement; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r031_requests_total`
- `voucher_system_r031_failures_total`
- `voucher_system_r031_operation_duration_ms`
- `voucher_system_r031_business_events_total`
...
- Custom event `R031.PlanosQuotasBillingEEntitlements.OperationCompleted`.
- Custom event `R031.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para planos comerciais, quotas e habilitação de funcionalidades.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para planos comerciais, quotas e habilitação de funcionalidades estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/PlanosQuotasBillingEntitlements/...`
- `src/VoucherSystem.Application/PlanosQuotasBillingEntitlements/Commands/...`
- `src/VoucherSystem.Application/PlanosQuotasBillingEntitlements/Queries/...`
- `src/VoucherSystem.Api/Endpoints/PlanosQuotasBillingEntitlements/...`
...
- `src/VoucherSystem.Workers/PlanosQuotasBillingEntitlements/...`
- `frontend/src/features/planos-quotas-billing-entitlements/...`
- `frontend/src/routes/...`
- `docs/requisitos/R031 - PLANOS-QUOTAS-BILLING-ENTITLEMENTS.md`
- `tests/VoucherSystem.UnitTests/PlanosQuotasBillingEntitlements/...`
- `tests/VoucherSystem.IntegrationTests/PlanosQuotasBillingEntitlements/...`
- `tests/VoucherSystem.ApiTests/PlanosQuotasBillingEntitlements/...`
- `frontend/tests/planos-quotas-billing-entitlements/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para planos comerciais, quotas e habilitação de funcionalidades.  
