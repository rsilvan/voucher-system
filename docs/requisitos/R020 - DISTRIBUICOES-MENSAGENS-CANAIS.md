...
Este requisito tem como objetivo entregar incentivos e mensagens ao público correto no momento correto por canais próprios ou integrações. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
### Fora de escopo
- provedor real de email/SMS proprietário.
- marketing automation completo.
- editor visual avançado drag-and-drop.
- CDP externo.
...
3. Ator cria ou consulta recurso de distribuição, templates e canais com payload válido.
...
Fluxos alternativos:
1. Ator executa criação de distribuição usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r020:read`.
- Escrita: `r020:write`.
- Execução de ação crítica: `r020:execute`.
- Administração: `r020:admin`.
- Auditoria/exportação: `r020:audit` ou `r020:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de distribuição, templates e canais deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como criação de distribuição, seleção de audiência, publicação separada de envio só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de distribuição, templates e canais devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de distribuição, templates e canais.  
  Prioridade: Must.  
  Dependências: R005, R019, R021.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como criação de distribuição, seleção de audiência, publicação separada de envio, retry de canal, cancelamento.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de distribuição, templates e canais devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/distribuicoes-mensagens-canais` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `Distribution`: distribuição. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `MessageTemplate`: template. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ChannelConfig`: canal. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `DistributionTrigger`: gatilho. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `DeliveryAttempt`: tentativa. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `DeliveryLog`: log. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
...
- `GET /api/v1/distributions`  
  Objetivo: listar recursos de distribuição, templates e canais com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r020:read`.  
...
- `POST /api/v1/distributions`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r020:write`.  
...
- `GET /api/v1/distributions/{id}`  
  ...
  Autenticação exigida: scope `r020:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/distributions/{id}`  
  ...
  Autenticação exigida: scope `r020:write`.  
...
- **`distribution.created`**  
  Quando ocorre: após criação de distribuição ou mudança relevante no domínio de distribuição, templates e canais.  
...
Rotas:
- `/distributions`: listagem principal.
- `/distributions/new`: criação.
- `/distributions/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/distributions/:id/edit`: edição controlada por estado.
- `/distributions/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de distribuição, templates e canais não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar criação de distribuição; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r020_requests_total`
- `voucher_system_r020_failures_total`
- `voucher_system_r020_operation_duration_ms`
- `voucher_system_r020_business_events_total`
...
- Custom event `R020.DistribuicoesMensagensECanais.OperationCompleted`.
- Custom event `R020.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para distribuição, templates e canais.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para distribuição, templates e canais estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/DistribuicoesMensagensCanais/...`
- `src/VoucherSystem.Application/DistribuicoesMensagensCanais/Commands/...`
- `src/VoucherSystem.Application/DistribuicoesMensagensCanais/Queries/...`
- `src/VoucherSystem.Api/Endpoints/DistribuicoesMensagensCanais/...`
...
- `src/VoucherSystem.Workers/DistribuicoesMensagensCanais/...`
- `frontend/src/features/distribuicoes-mensagens-canais/...`
- `frontend/src/routes/...`
- `docs/requisitos/R020 - DISTRIBUICOES-MENSAGENS-CANAIS.md`
- `tests/VoucherSystem.UnitTests/DistribuicoesMensagensCanais/...`
- `tests/VoucherSystem.IntegrationTests/DistribuicoesMensagensCanais/...`
- `tests/VoucherSystem.ApiTests/DistribuicoesMensagensCanais/...`
- `frontend/tests/distribuicoes-mensagens-canais/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para distribuição, templates e canais.  
