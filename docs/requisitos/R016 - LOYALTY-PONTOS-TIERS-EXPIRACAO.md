...
Este requisito tem como objetivo criar programas de relacionamento que recompensem comportamento, aumentem retenção e controlem pontos/tier de forma auditável. O valor para o produto é criar uma base operacional confiável para campanhas promocionais, validações, resgates, análises e integrações API-first. Para os usuários, reduz dependência de desenvolvimento sob demanda, aumenta rastreabilidade, melhora governança e permite operação segura em ambiente multi-tenant.
...
- expiração por campanha ou regra.
- tiers por saldo ou acumulado.
- upgrade/downgrade/prolongamento.
- transferência e ajustes autorizados.
- extrato e buckets expirantes.

### Fora de escopo
- catálogo material de recompensas avançado.
- campanhas referral.
- gift cards monetários.
- gamificação social.
...
3. Ator cria ou consulta recurso de programas de fidelidade, pontos e tiers com payload válido.
...
Fluxos alternativos:
1. Ator executa join do membro usando API key server-side; sistema aplica scopes e mesmo fluxo de auditoria.
...
- Leitura: `r016:read`.
- Escrita: `r016:write`.
- Execução de ação crítica: `r016:execute`.
- Administração: `r016:admin`.
- Auditoria/exportação: `r016:audit` ou `r016:export`.
...
- **RN-001 — Isolamento tenant-aware**  
  Descrição: todo recurso de programas de fidelidade, pontos e tiers deve pertencer a uma organização e, quando aplicável, a um projeto.  
...
- **RN-002 — Estado permitido antes da operação crítica**  
  Descrição: operações como join do membro, acúmulo de pontos, consumo por FIFO de expiração só podem ocorrer em estados explicitamente permitidos.  
...
- **RN-004 — Auditoria obrigatória**  
  Descrição: alterações administrativas e ações críticas de programas de fidelidade, pontos e tiers devem gerar audit log append-only.  
...
- **RF-001 — CRUD/listagem do domínio**  
  Descrição: disponibilizar criação, edição permitida, consulta, listagem paginada e exclusão lógica dos recursos de programas de fidelidade, pontos e tiers.  
  Prioridade: Must.  
  Dependências: R004, R007, R013.  
...
- **RF-002 — Execução das ações críticas**  
  Descrição: suportar ações de domínio como join do membro, acúmulo de pontos, consumo por FIFO de expiração, cálculo de tier, job de expiração.  
  Prioridade: Must.  
...
- **RF-005 — Integração API-first**  
  Descrição: todos os fluxos essenciais de programas de fidelidade, pontos e tiers devem ser executáveis por API antes ou junto do portal.  
  Prioridade: Must.  
  Dependências: R003.  
...
- **RT-009 — Frontend React/TypeScript/Vite**: implementar feature isolada em `frontend/src/features/loyalty-pontos-tiers-expiracao` com client tipado, hooks, rotas protegidas e tratamento de erro padronizado.
...
- `LoyaltyProgram`: programa. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `LoyaltyMember`: membro. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `LoyaltyCard`: cartão. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `EarningRule`: regra de ganho. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `PointsLedgerEntry`: lançamento de pontos. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `amount numeric(18,6)`, `currency/points_type text`, `reference_id uuid`, `metadata jsonb`, `created_at timestamptz`, `row_version`.
- `Tier`: nível. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `status text`, `metadata jsonb`, `created_at timestamptz`, `updated_at timestamptz`, `row_version xmin/bytea`.
- `ExpiringPointsBucket`: bucket. Campos sugeridos: `id uuid PK`, `account_id uuid`, `project_id uuid`, `amount numeric(18,6)`, `currency/points_type text`, `reference_id uuid`, `metadata jsonb`, `created_at timestamptz`, `row_version`.
...
- `GET /api/v1/loyalty`  
  Objetivo: listar recursos de programas de fidelidade, pontos e tiers com paginação server-side.  
  Autenticação exigida: Bearer JWT ou API key com scope `r016:read`.  
...
- `POST /api/v1/loyalty`  
  ...
  Autenticação exigida: Bearer JWT ou API key server-side com scope `r016:write`.  
...
- `GET /api/v1/loyalty/{id}`  
  ...
  Autenticação exigida: scope `r016:read` e acesso ao `account_id/project_id`.  
...
- `PATCH /api/v1/loyalty/{id}`  
  ...
  Autenticação exigida: scope `r016:write`.  
...
- **`loyalty_member.joined`**  
  Quando ocorre: após join do membro ou mudança relevante no domínio de programas de fidelidade, pontos e tiers.  
...
- Ação: join do membro.  
  ...
  Entidade afetada: recurso principal de programas de fidelidade, pontos e tiers.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: acúmulo de pontos.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de programas de fidelidade, pontos e tiers.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: consumo por FIFO de expiração.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de programas de fidelidade, pontos e tiers.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: High.
- Ação: cálculo de tier.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de programas de fidelidade, pontos e tiers.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
  Severidade: Medium.
- Ação: job de expiração.  
  Ator: usuário autenticado, API credential ou worker identificado.  
  Entidade afetada: recurso principal de programas de fidelidade, pontos e tiers.  
  Antes/depois: obrigatório para alteração; snapshot resumido para criação/execução.  
  correlationId: obrigatório.  
...
Rotas:
- `/loyalty`: listagem principal.
- `/loyalty/new`: criação.
- `/loyalty/:id`: detalhe com abas de visão geral, configuração, timeline e auditoria.
- `/loyalty/:id/edit`: edição controlada por estado.
- `/loyalty/:id/actions`: ações críticas, quando fizer sentido.
...
- Campos obrigatórios do domínio de programas de fidelidade, pontos e tiers não podem ser nulos ou vazios.
...
- **Unitário — regras de domínio**  
  Given recurso em estado permitido; When executar join do membro; Then estado, eventos e validações devem ser consistentes.
...
Métricas:
- `voucher_system_r016_requests_total`
- `voucher_system_r016_failures_total`
- `voucher_system_r016_operation_duration_ms`
- `voucher_system_r016_business_events_total`
...
- Custom event `R016.LoyaltyPontosTiersEExpiracao.OperationCompleted`.
- Custom event `R016.BusinessRuleViolation`.
...
- Risco: regras de estado mal definidas para programas de fidelidade, pontos e tiers.  
  Impacto: médio.  
  Probabilidade: média.  
...
- Todos os endpoints definidos para programas de fidelidade, pontos e tiers estão implementados, documentados em OpenAPI e protegidos por autenticação/autorização.
...
- `src/VoucherSystem.Domain/LoyaltyPontosTiersExpiracao/...`
- `src/VoucherSystem.Application/LoyaltyPontosTiersExpiracao/Commands/...`
- `src/VoucherSystem.Application/LoyaltyPontosTiersExpiracao/Queries/...`
- `src/VoucherSystem.Api/Endpoints/LoyaltyPontosTiersExpiracao/...`
...
- `src/VoucherSystem.Workers/LoyaltyPontosTiersExpiracao/...`
- `frontend/src/features/loyalty-pontos-tiers-expiracao/...`
- `frontend/src/routes/...`
- `docs/requisitos/R016 - LOYALTY-PONTOS-TIERS-EXPIRACAO.md`
- `tests/VoucherSystem.UnitTests/LoyaltyPontosTiersExpiracao/...`
- `tests/VoucherSystem.IntegrationTests/LoyaltyPontosTiersExpiracao/...`
- `tests/VoucherSystem.ApiTests/LoyaltyPontosTiersExpiracao/...`
- `frontend/tests/loyalty-pontos-tiers-expiracao/...`

## 19. Plano incremental de implementação

- **Iteração 1 — Modelo de domínio e persistência**  
  Objetivo: criar entidades, enums, constraints, migrations e repositórios/query services para programas de fidelidade, pontos e tiers.  
